...
export function EndedRoomCard({ id, dateLabel, startAt, endAt, topic, host, liveUsers = [], onView }: EndedRoomCardProps) {
  ...
  return (
    <article
      className="border rounded-lg p-4 bg-white border-[#E9EDF6] transition-shadow"
    ...
    >
      <header className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          <span className="text-[12px] text-[#6B7280]">Finalizada</span>
        </div>
      </header>

      <h3 id={`room-${id}-title`} className="text-[16px] font-medium text-[#16161A] mb-1">
        {topic}
      </h3>
      <p className="text-[12px] font-normal text-[#394150] mb-3">{host}</p>

      <div className="flex items-center gap-3 mb-4">
        {safeUsers.length === 0 ? (
          <span className="text-xs text-[#808DB2]">Sem participantes</span>
        ) : (
          <>
      ...
      <div>
        <button
          type="button"
          onClick={() => onView?.(id)}
          className="w-full flex items-center justify-center gap-2 bg-white text-[#475569] px-3 py-2 rounded-md border border-[#E6E9EE] text-sm cursor-pointer"
        >
          <span>Ver detalhes da sala</span>
        </button>
      </div>
    </article>
  );
