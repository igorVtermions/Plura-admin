...
function adaptRoom(r: RoomApi): RoomUI | null {
  ...
  const users =
    Array.isArray(r["liveUsers"]) || Array.isArray(r["users"])
      ? ((r["liveUsers"] ?? r["users"]) as unknown[]).filter(
          (u): u is User => typeof u === "object" && u != null,
        )
      : [];
...
export function RoomsControl() {
  ...
  const rooms = useMemo(
    () => (tab === "live" ? liveRooms : soonRooms),
    [tab, liveRooms, soonRooms],
  );
  ...
  return (
    <section className="mt-8 p-1 pb-0 rounded-lg bg-white flex flex-col overflow-hidden h-full min-h-0">
      ...
      <div
        key={`${tab}-${page}`}
        className={`grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mb-0 ${
      ...
      >
        {loading ? (
          Array.from({ length: perPage }).map((_, i) => (
            <div
              key={i}
              className="border rounded-lg p-4 bg-[#FBFBFF] border-[#E9EDF6] animate-pulse h-[180px]"
            />
          ))
        ) : rooms.length === 0 ? (
      ...
      {totalPages > 1 && (
        <div
          className="flex items-center justify-center flex-shrink-0 mt-4 md:hidden"
