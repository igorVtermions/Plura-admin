...
type Notification = { id: string | number; title?: string; body?: string; read?: boolean; createdAt?: string };
            ...
            {notifications.length === 0 ? (
              ...
              notifications.map((n) => (
                <div key={n.id} className={`p-3 hover:bg-slate-50 ${n.read ? "" : "bg-slate-50"}`}>
                  ...
                  {n.createdAt && <div className="text-xs text-muted-foreground mt-1">{String(n.createdAt)}</div>}
                </div>
              ))
            )}
          </div>
