...
export default function InstructorActivitySection({ sessions, total, hostName }: Props) {
  ...
  try {
    ...
    for (const s of sessions) {
      const live = Boolean(s.isLive) || (s.status ? ["live", "ao vivo", "in progress"].includes((s.status ?? "").toLowerCase()) : false);
      const endDate = s.endTime ? new Date(s.endTime) : null;
      const ended = endDate && !Number.isNaN(endDate.getTime()) && endDate.getTime() < now.getTime();
        ...
        {Array.isArray(sessions) && sessions.length > 0 ? (
          sessions.map((session) => {
            ...
            if (startDate && endDate && !Number.isNaN(startDate.getTime()) && !Number.isNaN(endDate.getTime())) {
              ...
              live = Boolean(session.isLive) || (session.status ? ["live", "ao vivo", "in progress"].includes((session.status ?? "").toLowerCase()) : false);
            }
            ...
            const ended = !live && endDate && !Number.isNaN(endDate.getTime()) && endDate.getTime() < now.getTime();
